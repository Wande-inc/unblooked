const fpPromise = import(
  "https://cdn.jsdelivr.net/npm/@fingerprintjs/fingerprintjs@4.6/+esm"
...
fpPromise
  ...
  .then(async (result) => {
    ...
    try {
      // Fetch the JSON file and await the response
      const response = await fetch(
        "https://cdn.jsdelivr.net/gh/Wande-inc/unblooked@main/assets/site/ids.json?timestamp=" +
          Date.now()
      );
      ...
      // Validate and check if visitorId exists
      if (data.ids && Array.isArray(data.ids)) {
        if (data.ids.includes(visitorId)) {
          makeSite();
        } else {
          makeError(visitorId); // Fixed: Pass visitorId instead of undefined id
        }
      } else {
        console.error("Invalid JSON format: 'ids' should be an array.");
      }
    } catch (error) {
...
<div>
    <span id="locked"><i class="fa fa-lock"></i>This site is locked</span>
    <span id="fp">${id}</span>
</div>
  `;
}
